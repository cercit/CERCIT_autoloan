...
export async function getSession(): Promise<{ user?: UserProfile | undefined; session?: any } | null> {
  try {
    const { data, error } = await supabase.auth.getSession();
    const user = await getCurrentUser();
    if (error || !data.session) return { user: user ?? undefined, session: null };
    return { user: user ?? undefined, session: data.session };
  } catch (e) {
    const user = await getCurrentUser();
    return { user: user ?? undefined, session: null };
  }
}
