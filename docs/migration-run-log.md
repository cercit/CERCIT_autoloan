...
| 013 | `013_pii_enforce_no_plaintext.sql` | ☐ | Blocks any future plain-text PAN or mobile |
| 014 | `014_customer_employment_fields.sql` | ☐ | Employment fields the assessment needs |
| 015 | `015_tenants_and_feature_flags.sql` | ☐ | Tenants and the module switches (all off) |
| 016 | `016_versioned_policy.sql` | ☐ | Versioned, approved credit policy with its guard rules |
| 017 | `017_seed_policy_2026_08.sql` | ☐ | Today's rules stored as the active baseline, version 2026.08 |
| 018 | `018_permission_checks.sql` | ☐ | Permission checks in the database, not just the screens |
| 019 | `019_submit_application_permission.sql` | ☐ | Submitting an application needs the right to create one |
| 021 | `021_draft_policy_2026_09.sql` | ☐ | The unified rules stored as draft 2026.09 (not live until approved) |
