...
| 19 Sep 2026 | CC2.2 | Rate Grid screen, behind `credit_control`: every rate, cap and fee now comes from the approved policy version, with the version and its start date shown, and a Propose a change button that leads to the Policy Rules screen. Because recommendations are still priced from the old rate tables until FD5, any figure where those tables differ from the approved version is listed in a warning rather than hidden |
